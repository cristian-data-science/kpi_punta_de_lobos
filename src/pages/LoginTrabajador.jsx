...
import { getPersonaByRut } from '@/services/supabaseHelpers'
import { validarRut, formatRut, cleanRut } from '@/utils/rutValidator'
...
const LoginTrabajador = () => {
  ...
  const { loginTrabajador } = useTrabajador()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Buscar persona por RUT (pasar el RUT original con formato)
      // La función getPersonaByRut manejará las diferentes búsquedas
      const { data, error: supabaseError } = await getPersonaByRut(rut)

      if (supabaseError || !data) {
        setError('RUT no encontrado. Contacta con administración.')
        setLoading(false)
        return
      }
      ...
      navigate('/trabajador/turnos')
    } catch (err) {
      setError('Error al conectar con el servidor. Intenta nuevamente.')
      console.error('Error en login trabajador:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleRutChange = (e) => {
    ...
    if (value.length > rut.length) {
      // Si está agregando caracteres, formatear
      const formatted = formatRut(value)
      setRut(formatted)
    } else {
      // Si está borrando, dejar como está
      setRut(value)
    }
  }
    ...
    <div className="login-page">
      ...
      <div className="login-container">
        <div className="login-header">
          ...
          <p>Ingresa tu RUT para acceder</p>
        </div>

        <form onSubmit={handleSubmit} id="loginTrabajadorForm">
          ...
          <button type="submit" className="btn-login" disabled={loading}>
            {loading ? 'Verificando...' : 'Ver Calendario'}
          </button>
        </form>

        <div className="signup-link">
          <a href="/">Acceso Administrativo</a>
        </div>
      </div>
    </div>
  )
